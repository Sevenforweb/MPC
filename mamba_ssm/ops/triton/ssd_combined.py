...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
        ...
        try:
            ...
            print(f"Assertion failed: zxbcdt.shape {zxbcdt.shape} does not match expected shape {(batch, seqlen, 2 * d_nonssm + 2 * dim + 2 * ngroups * dstate + nheads)}")
            print(f"Variables: batch={batch}, seqlen={seqlen}, d_nonssm={d_nonssm}, dim={dim}, ngroups={ngroups}, dstate={dstate}, nheads={nheads}")
        ...
        xBC_conv = rearrange(
            causal_conv1d_cuda.causal_conv1d_fwd(rearrange(xBC, "b s d -> b d s"),
                                                 conv1d_weight, None, None, activation in ["silu", "swish"]),
            "b d s -> b s d"
        )
